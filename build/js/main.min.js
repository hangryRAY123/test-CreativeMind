...
window.addEventListener("DOMContentLoaded", function () {
  if (window.matchMedia("(min-width: 1440px)").matches) {
    ...
    // on resize, things must be recalculated
    var directionalSnap = function directionalSnap(timeline) {
    ...
    var sections = _gsapMin.gsap.utils.toArray(".horizontal-wrapper section"),
      nav = _gsapMin.gsap.utils.toArray(".header__nav li"),
      getMaxWidth = function getMaxWidth() {
        return sections.reduce(function (val, section) {
          return val + section.offsetWidth + 7;
        }, 1);
      },
      maxWidth = getMaxWidth(),
      scrollSpeed = 4,
    ...
    init();
    _ScrollTriggerMin.ScrollTrigger.addEventListener("refreshInit", init);
    document.querySelectorAll(".header__nav-link[href]").forEach(function (a) {
  ...
  if (window.matchMedia("(min-width: 1024px)").matches && window.matchMedia("(max-width: 1439px)").matches) {
    var _init = function _init() {
      _gsapMin.gsap.set(_sections, {
        x: 0
      });
      _maxWidth = _getMaxWidth();
      var position = 0,
        distance = _maxWidth - window.innerWidth;
      // add a label for each section to the timeline (for "labelsDirectional" functionality):
      _tl.add("label0", 0);
      _sections.forEach(function (section, i) {
        var progress = position;
        position += section.offsetWidth / distance;
        _tl.add("label" + (i + 1), position);
      });
    };
    // on resize, things must be recalculated
    var _directionalSnap = function _directionalSnap(timeline) {
      return function (value, st) {
        if (Date.now() - _lastScrollTween < 1650) {
          // recently finished doing a tweened scroll (clicked link), so don't do any snapping.
          return _snapProgress;
        }
        var a = [],
          labels = timeline.labels,
          duration = timeline.duration(),
    ...
    var _getSamePageAnchor = function _getSamePageAnchor(link) {
      if (link.protocol !== window.location.protocol || link.host !== window.location.host || link.pathname !== window.location.pathname || link.search !== window.location.search) {
        return false;
      }
      return link.hash;
    };
    var _scrollToHash = function _scrollToHash(hash, e) {
      var elem = hash ? document.querySelector(hash) : false;
      if (elem) {
        if (e) e.preventDefault();
        _gsapMin.gsap.to(window, {
          ...
          scrollTo: {
            ...
            onEnter: function onEnter(e) {
              if (elem.id == "products" || elem.id == "footer") {
                _header.classList.add("fill");
              } else {
                _header.classList.remove("fill");
              }
            }
          }
        });
    ...
    var _intro = document.querySelector(".intro");
    var _about = document.querySelector(".about");
    var _team = document.querySelector(".team");
    document.addEventListener("scroll", function () {
      if (window.visualViewport.pageTop > 200) {
        _intro.classList.add("opacity");
        _about.classList.remove("opacity");
      } else {
        _intro.classList.remove("opacity");
        _about.classList.add("opacity");
      }
      if (window.visualViewport.pageTop > 750) {
        _team.classList.remove("opacity");
        _about.classList.add("opacity");
      } else {
        _team.classList.add("opacity");
      }
    });
    ...
    _gsapMin.gsap.registerPlugin(_ScrollTriggerMin.ScrollTrigger);
    var _sections = _gsapMin.gsap.utils.toArray(".horizontal-wrapper section"),
      _nav = _gsapMin.gsap.utils.toArray(".header__nav li"),
      _getMaxWidth = function _getMaxWidth() {
        return _sections.reduce(function (val, section) {
          return val + section.offsetWidth + 6;
        }, 1);
      },
      _maxWidth = _getMaxWidth(),
      _scrollSpeed = 4,
      _snapProgress,
      _lastScrollTween = Date.now(),
      _tl = _gsapMin.gsap.timeline();
    _tl.to(_sections, {
      x: function x() {
    ...
    _ScrollTriggerMin.ScrollTrigger.create({
      ...
      scrub: 1,
      snap: {
        snapTo: _directionalSnap(_tl),
        duration: 0.5
      },
      end: function end() {
        return "+=" + _maxWidth / _scrollSpeed;
      },
      invalidateOnRefresh: true
    });
    _init();
    _ScrollTriggerMin.ScrollTrigger.addEventListener("refreshInit", _init);
    document.querySelectorAll(".header__nav-link[href]").forEach(function (a) {
      a.addEventListener("click", function (e) {
        _scrollToHash(_getSamePageAnchor(a), e);
      });
    });
    ...
    var _footerTimeline = _gsapMin.gsap.timeline({
      paused: true
    });
    _footerTimeline.fromTo(".footer", {
      yPercent: -75,
      duration: 3,
      ease: "power2.out"
    }, {
    ...
    var _panels = _gsapMin.gsap.utils.toArray("section");
  ...
  if (window.matchMedia("(min-width: 768px)").matches && window.matchMedia("(max-width: 1023px)").matches) {
    var _getSamePageAnchor2 = function _getSamePageAnchor2(link) {
      if (link.protocol !== window.location.protocol || link.host !== window.location.host || link.pathname !== window.location.pathname || link.search !== window.location.search) {
        return false;
      }
      return link.hash;
    };
    var _scrollToHash2 = function _scrollToHash2(hash, e) {
      var elem = hash ? document.querySelector(hash) : false;
      if (elem) {
        if (e) e.preventDefault();
        _gsapMin.gsap.to(window, {
    ...
    document.querySelectorAll(".header__nav-link[href]").forEach(function (a) {
      a.addEventListener("click", function (e) {
        _scrollToHash2(_getSamePageAnchor2(a), e);
      });
    });
    _scrollToHash2(window.location.hash);
  }
  if (window.matchMedia("(max-width: 767px)").matches) {
    var _getSamePageAnchor3 = function _getSamePageAnchor3(link) {
      if (link.protocol !== window.location.protocol || link.host !== window.location.host || link.pathname !== window.location.pathname || link.search !== window.location.search) {
        return false;
      }
      return link.hash;
    };
    var _scrollToHash3 = function _scrollToHash3(hash, e) {
      var elem = hash ? document.querySelector(hash) : false;
      if (elem) {
        if (e) e.preventDefault();
        _gsapMin.gsap.to(window, {
    ...
    document.querySelectorAll(".header__nav-link[href]").forEach(function (a) {
      a.addEventListener("click", function (e) {
        _scrollToHash3(_getSamePageAnchor3(a), e);
      });
    });
    _scrollToHash3(window.location.hash);
  }
